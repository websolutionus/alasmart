    ...
    //mini cart function start
        ...
        function cartItem(){
            $.ajax({
                ...
                success:function(response){
                    let cartItem="";
                    $('#cartTotal').text(response.cartTotal * response.currencyRate);
               ...
               success:function(data){
                if(data.coupon_name){
                    if (data.currencyPosition == 'right') {
                        $('#calprice').html(`
                        <p class="subtotal">{{__('user.Subtotal')}} <span><span id="cartTotal">${data.sub_total}</span>${data.currencyIcon}</span></p>
                        ...
                        <p class="discount">{{__('user.Discount')}} <span>(-)${data.discount_amount} ${data.currencyIcon}</span></p>
                        <p class="total">{{__('user.Total')}} <span>${data.total_amount}<span>${data.currencyIcon}</span></span></p>
                        <a class="common_btn" href="{{ route('checkout') }}">{{__('user.Proceed to Checkout')}}</a>
                    `);
                    } else {
                        $('#calprice').html(`
                        <p class="subtotal">{{__('user.Subtotal')}} <span>${data.currencyIcon}<span id="cartTotal">${data.sub_total}</span></span></p>
                        <p class="subtotal">{{__('user.Coupon')}} <span>${data.coupon_name} <button type="submit" class="btn btn-danger btn-sm" onclick="couponRemove()"><i class="fa fa-times"></i></button></span></p>
                        <p class="discount">{{__('user.Discount')}} <span>(-)${data.currencyIcon} ${data.discount_amount}</span></p>
                        <p class="total">{{__('user.Total')}} <span><span>${data.currencyIcon}</span>${data.total_amount}</span></p>
                        <a class="common_btn" href="{{ route('checkout') }}">{{__('user.Proceed to Checkout')}}</a>
                    `);
                    }
                    ...
                    if (data.currencyPosition == 'right') {
                        ...
                        <p class="total">{{__('user.Total')}} <span><span><span>${data.sub_total}</span>${data.currencyIcon}</span></p>
                        <a class="common_btn" href="{{ route('checkout') }}">{{__('user.Proceed to Checkout')}}</a>
                    `);
                    } else {
                        $('#calprice').html(`
                        ...
                        <p class="total">{{__('user.Total')}} <span><span>${data.currencyIcon}<span>${data.sub_total}</span></span></p>
                        <a class="common_btn" href="{{ route('checkout') }}">{{__('user.Proceed to Checkout')}}</a>
                    `);
                    }
                }
