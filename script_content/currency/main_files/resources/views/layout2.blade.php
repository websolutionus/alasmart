                ...
                <ul class="navbar-nav m-auto">
                    <li class="nav-item">
                        ...
                        <ul class="wsus__droap_menu">
                            ...
                            <li><a class="active" href="{{ route('home',['theme' => 2]) }}">{{__('user.home two')}}</a></li>
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    $.each(response.carts, function(key, value){
                        let itemPrice = response.currencyRate * value.price;
                        if (response.currencyPosition == 'right') {
                            cartItem+=`<tr>
                                    <td class="img">
                                    ...
                                    <td class="price">
                                        <p>${itemPrice+response.currencyIcon}</p>
                                    </td>
                                    <td class="discount">
                            ...
                            cartItem+=`<tr>
                                    ...
                                    <td class="price">
                                        <p>${response.currencyIcon+itemPrice}</p>
                                    </td>
                                    <td class="discount">
                                        <p>${value.options.category}</p>
                                    </td>
        ...
        function couponCalculation(){
            $.ajax({
               ...
               success:function(data){
                if(data.total){
                    if (data.currencyPosition == 'right') {
                        $('#calprice').html(`
                        <p class="subtotal">{{__('user.Subtotal')}} <span><span id="cartTotal">${data.total * data.currencyRate}</span>${data.currencyIcon}</span></p>
                        <p class="discount">{{__('user.Discount')}} <span>(-)0 ${data.currencyIcon}</span></p>
                        <p class="total">{{__('user.Total')}} <span><span><span>${data.total * data.currencyRate}</span>${data.currencyIcon}</span></p>
                        <a class="common_btn" href="{{ route('checkout') }}">{{__('user.Proceed to Checkout')}}</a>
                    `);
                    } else {
                        $('#calprice').html(`
                        <p class="subtotal">{{__('user.Subtotal')}} <span>${data.currencyIcon}<span id="cartTotal">${data.total * data.currencyRate}</span></span></p>
                        <p class="discount">{{__('user.Discount')}} <span>(-)${data.currencyIcon} 0</span></p>
                        <p class="total">{{__('user.Total')}} <span><span>${data.currencyIcon}<span>${data.total * data.currencyRate}</span></span></p>
                        <a class="common_btn" href="{{ route('checkout') }}">{{__('user.Proceed to Checkout')}}</a>
                    `);
                    }
                    ...
                    if (data.currencyPosition == 'right') {
                        $('#calprice').html(`
                        <p class="subtotal">{{__('user.Subtotal')}} <span><span id="cartTotal">${data.sub_total * data.currencyRate}</span>${data.currencyIcon}</span></p>
                        ...
                        <p class="discount">{{__('user.Discount')}} <span>(-)${data.discount_amount * data.currencyRate} ${data.currencyIcon}</span></p>
                        <p class="total">{{__('user.Total')}} <span>${data.total_amount * data.currencyRate}<span>${data.currencyIcon}</span></span></p>
                        <a class="common_btn" href="{{ route('checkout') }}">{{__('user.Proceed to Checkout')}}</a>
                    `);
                    } else {
                        $('#calprice').html(`
                        <p class="subtotal">{{__('user.Subtotal')}} <span>${data.currencyIcon}<span id="cartTotal">${data.sub_total * data.currencyRate}</span></span></p>
                        <p class="subtotal">{{__('user.Coupon')}} <span>${data.coupon_name} <button type="submit" class="btn btn-danger btn-sm" onclick="couponRemove()"><i class="fa fa-times"></i></button></span></p>
                        <p class="discount">{{__('user.Discount')}} <span>(-)${data.currencyIcon} ${data.discount_amount * data.currencyRate}</span></p>
                        <p class="total">{{__('user.Total')}} <span><span>${data.currencyIcon}</span>${data.total_amount * data.currencyRate}</span></p>
                        <a class="common_btn" href="{{ route('checkout') }}">{{__('user.Proceed to Checkout')}}</a>
                    `);
                    }
                }
